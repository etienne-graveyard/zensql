...
it('parse and serialize a simple request', () => {
  expect(Serializer.serialize(Parser.parse(sql`SELECT foo FROM bar;`))).toEqual(
...
describe('parse and serialize all sort of queries without error with same result', () => {
  const QUERIES = [
    ...
    sql`SELECT * FROM foo WHERE id = 'azerty';`,
    ...
    sql`CREATE TABLE tab (id UUID PRIMARY KEY, name TEXT NOT NULL);`,
    sql`CREATE TABLE tab (id UUID PRIMARY KEY, other_id UUID REFERENCES other (id));`,
    sql`CREATE TABLE tab (id1 UUID, id2 UUID, PRIMARY KEY (id1, id2));`,
...
it('remove unecessary parentheses', () => {
  expect(
    Serializer.serialize(Parser.parse(sql`SELECT * FROM foo WHERE (id = ((6 * 6) / (2 + 75)));`))
  ).toEqual(sql`SELECT * FROM foo WHERE id = 6 * 6 / (2 + 75);`);
});
