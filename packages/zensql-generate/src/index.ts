...
type TableDefinitions = Array<Node<'CreateTableStatement'>>;
...
async function parseTables(tablesPath: string): Promise<TableDefinitions> {
...
async function convertQueries(tables: TableDefinitions, queriesFolder: string, outputFile: string) {
...
function convertQuery(tables: TableDefinitions, fileName: string, query: string): { name: string; body: string } {
  const parsed = Parser.parse(query);
  if (Array.isArray(parsed)) {
  ...
  const selectedTables = parsed.from.tables
    .map(fromExpre => resolveFromExpression(tables, fromExpre))
  ...
  const variables = resolveVariables(selectedTables, allColumns, columns, parsed.from.where);
...
function resolveFromExpression(tables: TableDefinitions, table: FromExpressionTable): Array<TableResolved> {
  if (NodeIs.LeftJoin(table)) {
    return resolveLeftJoin(tables, table);
  }
...
function resolveLeftJoin(tables: TableDefinitions, join: Node<'LeftJoin'>): Array<TableResolved> {
  // TODO: validate condition
  const left = resolveFromExpression(tables, join.left);
  const right = resolveFromExpression(tables, join.right);
  return [...left, ...right];
}
...
function findTable(tables: TableDefinitions, table: Identifier, alias: Identifier | null): TableResolved {
...
function generateOutputQuery(query: Node<'SelectStatement'>, variables: Array<VariableResolved>): string {
  ...
  if (query.from.where) {
    query.from.where = replaceVariable(query.from.where);
  }
